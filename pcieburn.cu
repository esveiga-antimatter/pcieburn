...
#define CUBLAS_CHECK(cmd)                                                      \
  do {                                                                         \
    ...
    if (s_ != CUBLAS_STATUS_SUCCESS)                                           \
      RANK_DIE("cuBLAS error %s:%d '%s' -> status %d", __FILE__, __LINE__,     \
               #cmd, (int)s_);                                                 \
  } while (0)
...
// host RAM: the rank writes up its own link and reads back down it. Where a
// PCIe switch allows direct peer access, the rank sends down its link to a peer
// and receives from peers over the same link. Either way its own link sees
// egress plus ingress.
//
...
static void issueCollective(const Config &cfg, DeviceBuffers &db,
                            ncclComm_t comm, cudaStream_t stream, int rank,
                            int nranks, size_t bytes) {
...
static void runRank(const Config &cfg, int rank, int nranks, int device,
  ...
  *db.stageU64 = 0;
  ...
  // whole job deadlocks. Reduce to the global minimum before allocating.
  {
    ...
    *db.stageU64 = local;
    CUDA_CHECK(cudaMemcpyAsync(db.reduceDev, db.stageU64, sizeof(*db.stageU64),
                               cudaMemcpyHostToDevice, stream));
    NCCL_CHECK(ncclAllReduce(db.reduceDev, db.reduceDev, 1, ncclUint64, ncclMin,
                             comm, stream));
    CUDA_CHECK(cudaMemcpyAsync(db.stageU64, db.reduceDev, sizeof(*db.stageU64),
    ...
    if (agreed != local)
      logf_("iteration count reduced %llu -> %llu to match slowest rank", local,
            agreed);
  ...
  while (!stopping) {
    for (size_t pi = 0; pi < cfg.precisions.size() && !stopping; pi++) {
      ...
      if (cfg.doCompare) {
        ...
        CUDA_CHECK(cudaMemcpyAsync(&db.stageInt[0], db.faultyDev, sizeof(int),
                                   cudaMemcpyDeviceToHost, stream));
        CUDA_CHECK(cudaMemcpyAsync(&db.stageInt[1], db.nanDev, sizeof(int),
                                   cudaMemcpyDeviceToHost, stream));
      } else {
        db.stageInt[0] = 0;
        db.stageInt[1] = 0;
      ...
      if (!cfg.noTensor && !hintedTensorCores && elapsed > cfg.duration / 2.0) {
  ...
  logf_("stopping cleanly after %lld iters, %lld GEMMs, %lld collectives "
        ...
        outerIter, cumGemms, cumColls, (double)cumCollBytes / 1073741824.0,
        cumFaulty, cumNans);
  ...
  cudaFree(db.aFP16);
  cudaFree(db.bFP16);
  cudaFree(db.faultyDev);
  cudaFree(db.nanDev);
  cudaFree(db.collSend);
  cudaFree(db.collRecv);
  cudaFree(db.stopDev);
  cudaFree(db.reduceDev);
  cudaFreeHost(db.stageInt);
...
static void usage(const char *argv0) {
  printf(
      "pcieburn — interleaved compute + NCCL/PCIe stress test (design Option "
      "B)\n"
      "\n"
      "Usage: %s [options]\n"
      "\n"
      "  --duration SEC        run length (default 60; the fault has "
      "reproduced\n"
      ...
      "  --collective NAME     allreduce | alltoall | sendrecv (default "
      "allreduce)\n"
      "  --coll-min SIZE       smallest collective, binary suffixes (default "
      "128M)\n"
      "  --coll-max SIZE       largest collective (default 1G). Both send and "
      "recv\n"
      "                        buffers are this size, so it costs 2x VRAM.\n"
      "  --coll-factor N       sweep multiplier (default 2)\n"
      "  --mem-frac F          fraction of remaining VRAM for C matrices "
      "(default 0.9)\n"
      "  --max-c-buffers N     cap the C buffer count (default 0 = unlimited)\n"
      "  --gpus LIST           comma list of device indices (default all "
      "visible)\n"
      "  --stream MODE         legacy | explicit (default legacy, as "
      "DCGM/gpu-burn)\n"
      "  --rank-stagger MS     delay rank N's GEMM burst by N*MS every pass, "
      "to\n"
      "                        de-correlate per-GPU power transients (default "
      "0).\n"
      "                        Diagnostic only: real tensor-parallel inference "
      "is\n"
      "                        synchronized, so nonzero values are LESS "
      "faithful.\n"
      "                        Use it to test whether correlated current steps "
      "across\n"
      "                        all GPUs are part of the failure mechanism.\n"
      "  --always-tensor       enable tensor-op math from the start, not at "
      "halfway\n"
      "  --no-tensor           never enable tensor-op math. Use with "
      "--precision\n"
      "                        single --matrix-dim 8192 to build a "
      "gpu-burn-equivalent\n"
      "                        arm: gpu-burn leaves cuBLAS at "
      "CUBLAS_DEFAULT_MATH and\n"
      "                        uses 8192-dim FP32 GEMMs, which dispatch ~65x "
      "fewer\n"
      ...
      "  --watchdog SEC        supervisor silence timeout (default 60, 0 = "
      "off)\n"
      "  --report-interval SEC CONSOLE progress cadence (default 1.0). The "
      "event\n"
      "                        log always records every pass regardless, so "
      "the\n"
      "                        CSV is a complete record and one precision "
      "cannot\n"
      "                        be under-sampled by interval aliasing.\n"
      "  --event-log PATH      append a CSV event log for telemetry "
      "correlation\n"
      "  --tag NAME            label recorded in the event log\n"
      "  -h, --help            this message\n"
      "\n"
      "Exit status: 0 all ranks clean; 1 usage/setup error; 2 a rank reported\n"
      "faults or NaNs; 3 a rank hung or died (the signature this test "
      "hunts).\n",
      argv0);
}
...
static bool parseArgs(int argc, char **argv, Config &cfg) {
  for (int i = 1; i < argc; i++) {
    ...
    if (!strcmp(a, "-h") || !strcmp(a, "--help")) {
      ...
      if (!next(&v))
        return false;
      if (!strcmp(v, "allreduce"))
        cfg.collective = COLL_ALLREDUCE;
      else if (!strcmp(v, "alltoall"))
        cfg.collective = COLL_ALLTOALL;
      else if (!strcmp(v, "sendrecv"))
        cfg.collective = COLL_SENDRECV;
      else {
      ...
      if (!next(&v))
        return false;
      if (!strcmp(v, "legacy"))
        cfg.explicitStream = false;
      else if (!strcmp(v, "explicit"))
        cfg.explicitStream = true;
      else {
...
int main(int argc, char **argv) {
...

  {
    ...
    logf_("  duration=%.0fs matrix_dim=%u precision=%s stream=%s", cfg.duration,
          cfg.matrixDim, precList.c_str(),
  ...
  while (liveRanks > 0) {
    ...
    for (int r = 0; r < nranks && sel > 0; r++) {
      ...
      if (m.magic != MSG_MAGIC) {
        ...
        eventLogWrite(cfg, "bad_message", r, &ranks[r].last, "protocol desync");
        close(ranks[r].readFd);
        ranks[r].readFd = -1;
        ranks[r].alive = false;
        liveRanks--;
      ...
      case MSG_READY:
        ...
        if (readyCount == nranks && !timersReset) {
          ...
          logf_("all %d ranks ready after %.1fs startup; load phase "
                "begins now",
                nranks, loadStart - startTime);
          eventLogWrite(cfg, "all_ready", -1, nullptr, "load phase start");
        }
        break;
        ...
        if (m.faulty || m.nans) {
          if (!sawFault)
            logf_("rank %d reported COMPUTE FAULTS "
                  "(faulty=%lld nan=%lld)",
                  r, (long long)m.faulty, (long long)m.nans);
          sawFault = true;
        }
        break;
      ...
      case MSG_DONE:
        ...
        logf_("rank %d done: %lld iters, %lld GEMMs, %lld collectives, "
              "%.2f GiB moved, faulty=%lld nan=%lld",
              r, (long long)m.outerIter, (long long)m.gemms, (long long)m.colls,
              (double)m.collBytes / 1073741824.0, (long long)m.faulty,
              (long long)m.nans);
      ...
      case MSG_ERROR:
        ranks[r].failed = true;
        logf_("rank %d ERROR at iter %lld: %s", r, (long long)m.outerIter,
              m.text);
    ...
    if (!timersReset && !killed &&
      ...
      logf_("only %d/%d ranks became ready within %.0fs — aborting", readyCount,
            nranks, STARTUP_DEADLINE_SEC);
      eventLogWrite(cfg, "startup_timeout", -1, nullptr, "not all ranks ready");
      sawHang = true;
    ...
    if (killed && nowSec() - killTime > grace) {
      logf_("ranks did not exit within %.0fs of teardown; escalating", grace);
      break;
    }
  }
  ...
  for (int r = 0; r < nranks; r++) {
    ...
    for (int attempt = 0; attempt < 100; attempt++) {
      ...
      if (got < 0) {
        ...
        logf_("waitpid(rank %d, pid %d) failed: %s", r, (int)ranks[r].pid,
              strerror(errno));
    ...
    logf_("rank %d (pid %d) will not die — likely stuck in the driver. "
          "A node power cycle may be required.",
          r, (int)ranks[r].pid);
  ...
  logf_("=== run complete in %.1fs (%.1fs startup, %.1fs under load) ===", wall,
        loadStart - startTime, loadWall);
  logf_("  ranks=%d GEMMs=%lld collectives=%lld faulty=%lld nan=%lld", nranks,
        totalGemms, totalColls, totalFaulty, totalNans);
  ...
  // disabled every remote byte crosses the bus twice via host RAM.
  {
    ...
    const double linkGBps = loadWall > 0.0 ? perRankLink / 1e9 / loadWall : 0.0;

    logf_("  PCIe traffic (%s, %d ranks, bw factor %.2fx, host-staged %.0fx):",
          collName(cfg.collective), nranks, bwFactor, PCIE_HOST_STAGING_MULT);
  ...
  for (int r = 0; r < nranks; r++) {
    ...
    for (size_t p = 0; p < cfg.precisions.size(); p++) {
      ...
      const int w = snprintf(buf + off, sizeof(buf) - (size_t)off, "  %s %.0f",
                             precName(pr), ranks[r].peakGflops[pr]);
      if (w < 0)
        break;
      off += w;
